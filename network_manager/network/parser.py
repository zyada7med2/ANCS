...
class IOSParser:
    ...
    INTERFACE_RE = re.compile(r"^interface\s+([a-zA-Z0-9/._-]+)\s*\n(.*?)(?=^interface|^router|^ip route|^line|^!|^end|\Z)", re.MULTILINE | re.DOTALL | re.IGNORECASE)
    ...
    DHCP_POOL_RE = re.compile(r"^ip dhcp pool\s+(\S+)\s*\n(.*?)(?=^!|^ip dhcp pool|^interface|^router|^ip route|^line|^end|\Z)", re.MULTILINE | re.DOTALL | re.IGNORECASE)
