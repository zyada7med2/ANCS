...
class SubnetCalculator(ctk.CTkToplevel):
    def __init__(self, parent):
        ...
        ctk.CTkLabel(self, text="Subnet Calculator", font=("Segoe UI", 20, "bold"), text_color="#00bfff").pack(pady=10)

        # tab view
        self.tabs = ctk.CTkTabview(self, width=850, height=500)
        ...
        # --- network info tab ---
        ctk.CTkLabel(self.tab_info, text="Network (e.g. 192.168.10.0/24):", font=("Segoe UI", 14)).grid(row=0, column=0, sticky="w", padx=10, pady=8)
        self.entry_net = ctk.CTkEntry(self.tab_info, width=220)
        self.entry_net.grid(row=0, column=1, padx=10)

        ctk.CTkLabel(self.tab_info, text="Number of Departments:", font=("Segoe UI", 14)).grid(row=1, column=0, sticky="w", padx=10, pady=8)
        self.entry_dept = ctk.CTkEntry(self.tab_info, width=220)
        self.entry_dept.grid(row=1, column=1, padx=10)

        ctk.CTkButton(self.tab_info, text="Next ➜", fg_color="#00bfff", command=self.create_dept_tab).grid(row=2, column=1, pady=20, sticky="e")

        # --- departments tab ---
        self.dept_frame = ctk.CTkScrollableFrame(self.tab_dept, fg_color="#1e1e1e", width=780, height=350)
        self.dept_frame.pack(pady=15)

        self.generate_btn = ctk.CTkButton(self.tab_dept, text="Generate Subnets", fg_color="#27ae60", command=self.calculate_subnets)
    ...
    def create_dept_tab(self):
        ...
        ctk.CTkLabel(self.dept_frame, text="Enter Department Info", font=("Segoe UI", 18, "bold"), text_color="#00bfff").pack(pady=10)

        for i in range(self.dept_count):
            row = ctk.CTkFrame(self.dept_frame, fg_color="transparent")
            row.pack(fill="x", pady=5)
            ctk.CTkLabel(row, text=f"Dept {i+1} Name:", width=100).pack(side="left", padx=6)
            name_entry = ctk.CTkEntry(row, width=180)
            name_entry.pack(side="left", padx=6)
            ctk.CTkLabel(row, text="Hosts:", width=70).pack(side="left", padx=6)
            hosts_entry = ctk.CTkEntry(row, width=100)
